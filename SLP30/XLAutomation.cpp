...
BOOL CXLAutomation::OpenExcelFile(CString szFileName/*, int nWorksheet=1*/)
{
	...
	ClearAllArgs();
	AddArgumentCString(L"Filename", 0, szFileName);
	if (!ExlInvoke(varg1.pdispVal, L"Open", &m_vargWorkbook, DISPATCH_PROPERTYGET, DISP_FREEARGS))
		return FALSE;

	//Now let's get the first worksheet of this workbook
	ClearAllArgs();
	AddArgumentInt2(NULL, 0, 1);
	if (!ExlInvoke(m_vargWorkbook.pdispVal, L"Worksheets", &m_vargWorksheet, DISPATCH_PROPERTYGET, DISP_FREEARGS))
		return FALSE;
	...
	m_pdispWorkbook = m_vargWorkbook.pdispVal;
	m_pdispWorksheet = m_vargWorksheet.pdispVal;

	return TRUE;
}

BOOL CXLAutomation::ChangeWorksheet(int nWorksheet)
{
	//Now let's get the first worksheet of this workbook
	ClearAllArgs();
