...
int CDataLinkManager::MakeExcelData(CString sPath)
{
	...
	for (int nIndex = 0; nIndex < nCount; nIndex++) {
		...
		if (bBlock) {
			if (pInfo->bRooftop)	//옥탑층
			{
				sTemp.Format(L"%s동 %d계단 Rooftop%s%s", pInfo->szBlock, pInfo->nStair, sInput, pInfo->szCircuitName);
			}
			else
			{
			...
			if (pInfo->bRooftop)	//옥탑층
			{
				sTemp.Format(L"%d계단 Rooftop%s%s", pInfo->nStair, sInput, pInfo->szCircuitName);
			}
			else
			{
	...
	for (int nIndex = 0; nIndex < nCount; nIndex++) {
		...
		{
			sTemp.Format(L"%s동 Rooftop", pBc->szBlock);
		}
		else
		{
